"""Phase 4 runtime param override — Sec 52 adaptivity for viva T1≠T2."""
...
# ex04-style inputs (salary + QP) — with 1.2M cap → tax 48000; with 1.8M → 0
_EX04_INPUTS = CalculateTaxRequestV1(
...
def test_is_sec52_cap_rule_matches_section_and_concept() -> None:
    assert is_sec52_cap_rule(
...
def test_seed_pre_amend_override_changes_current_pack_cap() -> None:
    settings = get_adaptive_tax_settings()
    assert not settings.param_override_path.is_file()

    baseline = load_tax_param_pack(param_set="current", settings=settings)
    assert baseline.relief_for_concept("qualifying_payment_cap").cap_amount == Decimal(
        "1800000"
    )
    ...
    overridden = load_tax_param_pack(param_set="current", settings=settings)
    assert overridden.relief_for_concept("qualifying_payment_cap").cap_amount == Decimal(
        "1200000"
    )
    # Explicit pre_amend pack is unchanged by override merge.
...
def test_reset_param_override_restores_ontology_current() -> None:
    ...
    assert load_tax_param_pack(param_set="current", settings=settings).relief_for_concept(
        "qualifying_payment_cap"
    ).cap_amount == Decimal("1200000")

    assert reset_param_override(settings=settings) is True
    assert not settings.param_override_path.is_file()
    assert load_tax_param_pack(param_set="current", settings=settings).relief_for_concept(
        "qualifying_payment_cap"
    ).cap_amount == Decimal("1800000")


def test_approve_writes_sec52_override_and_tax_changes() -> None:
    """Reset → T1; approve fixture Sec 52 → T2 ≠ T1."""
    settings = get_adaptive_tax_settings()
    seed_pre_amend_override(settings=settings)

    t1 = calculate(_EX04_INPUTS, kg=default_file_kg())
    assert t1.final_tax_lkr == "48000"
    ...
    assert result.param_override.rule_source_id == str(rule.id)
    doc = read_param_override(settings=settings)
    assert doc is not None
    assert doc["source"] == "sec52_approve"
    assert doc["rule_source_id"] == str(rule.id)

    t2 = calculate(_EX04_INPUTS, kg=default_file_kg())
    assert t2.final_tax_lkr == "0"
...
def test_reset_to_pre_amend_api(client: TestClient) -> None:
    ...
    assert body["qualifying_payment_cap"] == "1200000"
    ...
    assert calc.json()["final_tax_lkr"] == "48000"
